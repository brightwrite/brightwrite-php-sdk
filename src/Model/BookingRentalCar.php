...
class BookingRentalCar implements ArrayAccess
{
    ...
    protected static $swaggerFormats = [
        ...
        'provider' => null,
        'driverCount' => 'int32',
        'toddlerSeatCount' => 'int32',
        'infantChildSeatCount' => 'int32',
        'sIPPCode' => null,
        'vehicleType' => null
    ];
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['sIPPCode']) && (strlen($this->container['sIPPCode']) > 128)) {
            $invalid_properties[] = "invalid value for 'sIPPCode', the character length must be smaller than or equal to 128.";
        }

        $allowed_values = $this->getVehicleTypeAllowableValues();
        if (!in_array($this->container['vehicleType'], $allowed_values)) {
            $invalid_properties[] = sprintf(
                "invalid value for 'vehicleType', must be one of '%s'",
    ...
    public function valid()
    {
        ...
        if (strlen($this->container['sIPPCode']) > 128) {
            return false;
        }
        $allowed_values = $this->getVehicleTypeAllowableValues();
        if (!in_array($this->container['vehicleType'], $allowed_values)) {
    ...
    {
        if (!is_null($sIPPCode) && (strlen($sIPPCode) > 128)) {
            throw new \InvalidArgumentException('invalid length for $sIPPCode when calling BookingRentalCar., must be smaller than or equal to 128.');
        }
